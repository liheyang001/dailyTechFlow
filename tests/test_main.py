...
_MODULE_PATHS = [
    ...
    "src.content_generator.run",
    "src.image_generator.run",
    "src.voice_generator.run",
    "src.video_synthesizer.run",
    "src.publisher.run",
]
...
def _patch_runs(side_effects: dict | None = None):
    """
    Patch all 6 module run() functions.
...
def test_results_keys_order(tmp_path, monkeypatch):
    ...
    assert keys[-1].startswith("M6")


def test_six_results_returned(tmp_path, monkeypatch):
    _mock_setup(tmp_path, monkeypatch)
    with _patch_runs():
        results = main.run(DATE)
...
def test_one_module_exception_does_not_abort(tmp_path, monkeypatch):
    _mock_setup(tmp_path, monkeypatch)
    with _patch_runs({"image_generator": RuntimeError("boom")}) as mocks:
        results = main.run(DATE)
    m3_key = next(k for k in results if "image_generator" in k)
    assert "error" in results[m3_key]
    # M4-M6 仍然执行
...
def test_false_module_does_not_abort_pipeline(tmp_path, monkeypatch):
    _mock_setup(tmp_path, monkeypatch)
    with _patch_runs({"content_generator": False}):
        results = main.run(DATE)
    assert len(results) == 6
    # M3-M6 仍然执行并返回 ok
...
def test_all_modules_raise_still_returns_dict(tmp_path, monkeypatch):
    ...
    errors = {short: Exception("fail") for short in
              ["news_fetcher", "content_generator", "image_generator",
               "voice_generator", "video_synthesizer", "publisher"]}
...
def test_later_modules_run_after_early_failure(tmp_path, monkeypatch):
    ...
    with _patch_runs({"news_fetcher": RuntimeError("net error")}) as mocks:
        results = main.run(DATE)
    # M2-M6 全部被调用
    for name in ["content_generator", "image_generator",
                 "voice_generator", "video_synthesizer", "publisher"]:
...
def test_error_logged_in_run_log(tmp_path, monkeypatch):
    _mock_setup(tmp_path, monkeypatch)
    with _patch_runs({"voice_generator": ValueError("bad voice")}):
        main.run(DATE)
    content = (tmp_path / "output" / DATE / "run.log").read_text(encoding="utf-8")
    assert "bad voice" in content
