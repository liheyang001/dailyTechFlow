...
3. 数据真实性是铁律：每个数字都必须来自上方资料，绝不编造；资料里没有的数字宁可不写。但来源标注要克制——不要每句、每段都挂"（来源）"，那样很啰嗦；来源集中放在图表/表格底部和结尾的统一汇总，正文只在首次引用关键数字或直接引语时点一次即可。"""
    ...
    # 保留带 <style> 的完整 HTML：作邮件附件，手机点开用浏览器渲染、排版完整还原
